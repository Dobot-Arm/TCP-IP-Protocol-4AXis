...
- V1.9-2021/10/25:跟进PX的反馈，优化正逆解参数内容；新增获取错误码接口；实时反馈端口新增获取用户坐标、刀具坐标以及手系的参数；
...
​	根据设计，CR机器人可以开启29999、30003以及30004服务器端口；29999服务器端口(以下简称Dashboard端口)通过一发一收的方式负责接收一些简单的指令，即**Dashboard端口接收到客户端约定消息格式后会将结果反馈客户端**；30004服务器端口(以下简称实时反馈端口)**每20ms反馈机器人的信息**，30003服务器端口仅**接收客户端的约定消息格式，不做反馈**；(注意：30003端口的拆分成30003和30004端口预计在控制器3.5.2版本实现，3.5.1暂时还是一个30003端口进行实时反馈以及下发运动相关指令；)
...
- 可选参数数量：0/1/4  （不填参数默认都为0，返回-1；填一个参数默认为负载参数,返回0；填四个参数分别表示负载、X方向偏心距、Y方向偏心距以及Z方向偏心距，返回0;其他情况返回-1；）
...
​	30004端口即实时反馈端口(控制器3.5.2+版本支持)除了用于发送约定的运动相关协议外，还有其他功能，客户端每20ms能收到一次机器人如下表所示的信息。通过实时反馈端口每次收到的数据包有1440个字节，这些字节以标准的格式排列。下表是字节排列的顺序表。
...
如下表所示为30003实时反馈端口支持的运动命令协议，此端口仅接收命令不做反馈；
