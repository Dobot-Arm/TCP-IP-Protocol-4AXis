...
- 示例：
  ...
  | GetPose(User=1,Tool=0) | 0,{0.0,-246.0,847.0,-90.0},GetPose(User=1,Tool=0); | 立即指令--获取 用户坐标系索引号1 , 工具坐标系索引号0 下笛卡尔坐标系机械臂的实时位姿 |
  ​
...
- 示例：
  ...
  | 示例                          | 返回                                | 说明                                                         |
  | ----------------------------- | ----------------------------------- | ------------------------------------------------------------ |
  | RelMovLUser(10,10,10,0,0,0,0) | 0,{},RelMovLUser(10,10,10,0,0); | 队列指令--沿用户坐标系进行相对运动指令，末端运动方式为直线运动<br />X轴方向偏移10mm<br />Y轴方向偏移10mm<br />Z轴方向偏移10mm<br />R轴无变化<br />用户坐标系为0； |
